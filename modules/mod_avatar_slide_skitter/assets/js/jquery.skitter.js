...
(function($) {
	...
	var defaults = {
		...
		structure: 	 			  '<a href="#" class="prev_button">prev</a>'
								...
								+ '<div class="container_skitter">'
									+ '<div class="image">'
										+ '<a class="image_link_atual" href=""><img class="image_main" style="position: relative"/></a>'
	...
	$sk.fn.extend({
		...
		start: function()
		{
			...
			self.box_skitter.find('.image').css({height:self.settings.height_skitter});
			...
			self.box_skitter.find('.image .image_link_atual img').attr({'src': self.settings.image_atual});
			self.box_skitter.find('.image .image_link_atual img').css({left:self.settings.image_width, top:self.settings.image_height});
			
			img_link = self.box_skitter.find('.image .image_link_atual');
		...
		jumpToImage: function(imageNumber) 
		{
			...
			if (this.settings.is_animating == false) 
			{
				...
				this.box_skitter.find('.image .image_link_atual').attr({'href': this.settings.link_atual});
...
box_clone.css({left:_vleft+'px', top:_vtop+'px', width:width_box, height:height_box});		
		...
		animationSwapBars: function(options)
		{
			...
			for (i = 0; i < total; i++) {
				...
				// Old image
				var box_clone = this.getBoxCloneImgOld(image_old);
				box_clone.find('img').css({left:_vleft_image+image_width_old, top:0+image_height_old});
				box_clone.css({top:0, left:0, width:width_box, height:height_box});

				// Next image
				var box_clone_next = this.getBoxClone();
				box_clone_next.find('img').css({left:_vleft_image+this.settings.image_width, top:0+this.settings.image_height});
		...
		finishAnimation: function (options) 
		{
			...
			self.box_skitter.find('.image .image_link_atual img').css({left:this.settings.image_width, top:this.settings.image_height});
		...
		stopOnMouseOver: function () 
		{
			...
			self.box_skitter.hover(function() {
				...
				if (!self.settings.is_animating && self.settings.images_links.length > 1) {
					...
					self.box_skitter.find('.image .image_link_atual').attr({'href': self.settings.link_atual});
				}
		...
		setLinkAtual: function() {
			if (this.settings.link_atual != '#') {
				this.box_skitter.find('.image .image_link_atual').attr({'href': this.settings.link_atual, 'target': this.settings.target_atual});
			}
			else {
				this.box_skitter.find('.image .image_link_atual').removeAttr('href');
			}
		},
