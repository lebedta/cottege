...
class plgdjclassifiedspaymentdjcfPrzelewy24 extends JPlugin
{
	...
	function _notify_url()
	{
		...
		$p24_id_sprzedawcy = $this->params["p24_id"];
			...
			$p24_kwota = 0;
			if(strstr($item->pay_type, 'special,')){			
				$p24_kwota = $par->get('prom_price','0') * 100;
			}
			if(strstr($item->pay_type, 'cat')){			
				$p24_kwota += $item->c_price;
			}
		...
		if ($res[0]!='TRUE'){
			...
			die();
				...
				$query = "UPDATE #__djcf_payments SET status='Completed',transaction_id='".$p24_session_id."' "
						."WHERE item_id=".$id." AND method='djcfPrzelewy24'";
				$db->setQuery($query);
				$db->query();
	...
	function process($id)
	{
		...
		echo JText::_('PLG_DJCFPRZELEWY24_REDIRECTING_PLEASE_WAIT');
			...
			$form .='<br /><input type="hidden" id="p24_session_id" name="p24_session_id" value="'.$item->id.'">';
			...
			$form .='<br /><input type="hidden" name="p24_return_url_ok" value="'.JRoute::_(JURI::base().'index.php?option=com_djclassifieds&task=processPayment&ptype='.$this->params["plugin_name"].'&pactiontype=notify&id='.$item->id.'&Itemid='.$Itemid).'">';
			$form .='<br /><input type="hidden" name="p24_return_url_error" value="'.JRoute::_(JURI::base().'index.php?option=com_djclassifieds&task=paymentReturn&r=error&id='.$item->id.'&cid='.$item->cat_id.'&Itemid='.$Itemid).'">';
