...
{
	function plgdjclassifiedspaymentdjcfAuthorizeNET( &$subject, $config)
	{
	...
	function _notify_url()
	{
		...
		$msg_style='';
			...
			$card_num    = JRequest::getVar('card_no','0','','int');
		    ...
		    if ($response->approved) {
					$query = 'INSERT INTO #__djcf_payments ( item_id,user_id,method,  status)' .
						' VALUES ( "'.$id.'" ,"'.$user->id.'","'.$ptype.'" ,"Completed" )'
						;
				$db->setQuery($query);
				$db->query();
	...
	function onPaymentMethodList($val)
	{
		...
		<td><select name='exp_date' id='exp_date'>
						...
						$ADN_form .= "<input type='hidden' name='option' value='com_djclassifieds' /><input type='hidden' name='task' value='processPayment' /><input type='hidden' name='ptype' value='".$this->params["plugin_name"]."' /><input type='hidden' name='pactiontype' value='notify' />";
		...
		$html ='<table cellpadding="5" cellspacing="0" width="100%" border="0">
			...
			$html .='<td class="td1" width="160" align="center">
				...
				<td class="td3" width="100" align="center">
					<a  style="text-decoration:none;" onclick="adotnetSubmitForm();" href="javascript:void(0);"><img border="0" src="'.JURI::root().'components/com_djclassifieds/assets/images/buynow.png" width="89" height="28" ></a>
				</td>
			</tr>
		</table>';
		}
