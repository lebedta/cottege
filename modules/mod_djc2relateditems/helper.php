<?php
/**
 * @version $Id: helper.php 45 2011-11-07 11:37:41Z michal $
 * @package DJ-Catalog2
...
class modDjc2RelateditemsHelper extends JModel {
	var $_data = null;
	var $_cparams = null;
	var $_mparams = null;
	...
	/*function _getRelatedItemsQuery() {
		$option = JRequest::getVar('option');
		$view = JRequest::getVar('view');
		$id = JRequest::getVar('id');
		if ($option != 'com_djcatalog2' || $view != 'item' || !$id) {
		...
		$query = ' SELECT i.*, c.id AS ccategory_id, p.id AS pproducer_id, c.name AS category, p.name AS producer, p.published as publish_producer, img.fullname AS item_image, img.caption AS image_caption, '
				...
				. ' WHERE i.published = 1 AND i.id IN (SELECT related_item FROM #__djc2_items_related WHERE item_id='.(int)$id.')' 
