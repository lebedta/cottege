...
class Djcatalog2ModelRelateditems extends JModelList
{
	...
	public function assign($cid, $item_id) {
		if (count($cid) >= 0 || empty($item_id)) {
			$db = JFactory::getDbo();
			$db->setQuery('DELETE FROM #__djc2_items_related WHERE item_id=\''.$item_id.'\'');
			if ($db->query() && count($cid) > 0) {
