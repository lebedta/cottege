...
$saveOrder	= false;
$parent_id = JRequest::getVar('item_id',null);
?>
<form action="<?php echo JRoute::_('index.php?option=com_djcatalog2&view=relateditems&tmpl=component');?>" method="post" name="adminForm" id="adminForm">
	...
	<div class="clr"></div>
	<div class="filter-select fltrt">
		<button type="button" onclick="javascript:if (document.adminForm.boxchecked.value==0){alert('<?php echo JText::_('COM_DJCATALOG2_SELECT_ITEMS_TO_ASSIGN');?>');}else{ Joomla.submitbutton('relateditems.assign')}"><?php echo JText::_('JAPPLY'); ?></button>
		<button type="button" onclick="javascript:if (document.adminForm.boxchecked.value==0){alert('<?php echo JText::_('COM_DJCATALOG2_SELECT_ITEMS_TO_ASSIGN');?>');}else{ Joomla.submitbutton('relateditems.assignclose')}"><?php echo JText::_('JSAVE'); ?></button>
		<button type="button" onclick="window.parent.SqueezeBox.close();"><?php echo JText::_('JTOOLBAR_CLOSE'); ?></button>
	</div>
	<div>
