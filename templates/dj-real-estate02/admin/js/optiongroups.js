		...
		this.groups.each(function(el, index){
			...
			if (elements.length > 0) {
				...
				for (var i = 1; i < elements.length; i++) {
					...
					if (inputId) {
						inputId.getParent('li').setStyle('display', 'none');
					} else if (labelId) {
						labelId.getParent('li').setStyle('display', 'none');
					}
				}
			}
		}.bind(this));
		...
		document.id(this.name).addEvent('change', function(evt){
			...
			for (var i = 0; i < this.groupNames.length; i++) {
				...
				for (var j = 0; j < this.groupFields[group].length; j++) {
					...
					if (group == value) {
						if (inputId) {
							inputId.getParent('li').setStyle('display', '');
						} else if (labelId) {
							labelId.getParent('li').setStyle('display', '');
						}
					} else {
						if (inputId) {
							inputId.getParent('li').setStyle('display', 'none');
