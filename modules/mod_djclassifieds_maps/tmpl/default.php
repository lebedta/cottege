			    ...
			    {			    
				 document.getElementById("djmod_map_box").style.display='block';
					...
					foreach($items as $item){						
						?>
						var adr = '<?php echo $item->country.", ".$item->city; if($item->address!='' ){echo ", ".$item->address;}?>';
						djmod_geokoder.geocode({address: adr}, function (results, status){
				    		if(status == google.maps.GeocoderStatus.OK){
				    			<?php								
				    			$marker_txt = '<div style="width:200px;margin-bottom:0px"><div style="margin-bottom:5px;">';
				    			$marker_txt .= '<a href="index.php?option=com_djclassifieds&view=item&cid='.$item->cat_id.'&id='.$item->id.'&Itemid='.$itemid.' "><strong>'.$item->name.'</strong></a></div>';
				    			//$marker_txt .= '<strong>'.$item->name.'</strong></div>';
									$marker_txt .= $item->intro_desc.'<br />'; 								
									$marker_txt .= '<div style="margin-top:10px;">';
				    			...
				    			?>				    			
				    				djmodAddMarker(results[0].geometry.location,'<?php echo $marker_txt; ?>',icon);									
								}
								});					
						<?php 
				    		} ?>				    
