...
class DJClassifiedsControllerLicense extends JController {
	...
	public function edit(){
		...
		echo '<div class="lic_manager">';
			echo '<form action="index.php" method="post" name="adminForm" id="adminForm" >'
				...
				.'<div class="lic_drow"><span class="lic_drow_label">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_PRODUCT').'</span>'.$product.'</div>'
				.'<div class="lic_drow"><span class="lic_drow_label">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_DOMAIN').'</span>'.$u->getHost().'</div>';
	...
	function update_list(){
		...
		."FROM #__extensions WHERE element LIKE '%".$ext_name."%' OR element LIKE '%".$ext_name2."%' OR folder='djclassifieds' "
		."ORDER BY type ";
		$db->setQuery($query);
		...
		<div class="lic_extensions_box">	
				...
				foreach($ext_versions as $r){
								...
								echo '<tr><td>'.$ext->name.'</td>';
									...
									if($e[2]){
										if(version_compare($c_version,$l_version,'<')){
											echo '<div id="update_link_'.$ext->element.'" style="width:100px;"><span style="cursor:pointer; background: none repeat scroll 0 0 #F0F0F0;border: 1px solid silver;color: #000000;font-size: 10px;padding: 1px 5px;" onclick="make_update3(\''.$ext->element.'\',\''.$license.'\',\''.$c_version.'\');">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_UPDATE').'</span></div>';
										}else{
											echo JText::_('COM_DJCLASSIFIEDS_DJLIC_LATEST');	
										}		
									}else{
										if($e[3]){
					...
					if(!$exist){
						...
						echo '<td>'.$e[0].'</td><td>---</td><td>'.$e[1].'</td><td>';
							if($e[2]){
								echo '<div id="update_link_'.$e[0].'" style="width:100px;"><span style="cursor:pointer; background: none repeat scroll 0 0 #F0F0F0;border: 1px solid silver;color: #000000;font-size: 10px;padding: 1px 5px;" onclick="make_update3(\''.$e[0].'\',\''.$license.'\',\'0\');">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_INSTALL').'</span></div>';
							}else{
								if($e[4]){
			...
			<script type="text/javascript">
						...
						function make_update3(ext,license,version){	
							...
							$('update_link_'+ext+'').set('html','<?php echo JText::_('COM_DJCLASSIFIEDS_DJLIC_PLEASE_WAIT');?> <img src="<?php echo JURI::base().'components/'.JRequest::getString('option').'/images/loading.gif';?>" />'); 
							  ...
							  var myRequest = new Request({
							    ...
							    onRequest: function(){
							        myElement.set('html', '<div style="text-align:center;"><img style="margin-top:10px;" src="<?php echo JURI::base().'components/'.JRequest::getString('option').'/images/long_loader.gif';?>" /><br /><?php echo JText::_('COM_DJCLASSIFIEDS_DJLIC_LOADING');?></div>');
							    },
							    onSuccess: function(responseText){																
									$('update_link_'+ext+'').set('html','<?php echo JText::_('COM_DJCLASSIFIEDS_DJLIC_DONE');?> <img src="<?php echo JURI::base();?>templates/hathor/images/admin/tick.png" />');
									myElement.set('html', '');
