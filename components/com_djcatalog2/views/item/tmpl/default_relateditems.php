<?php
/**
* @version $Id: default_relateditems.php 73 2012-06-13 07:59:06Z michal $
* @package DJ-Catalog2
        ...
        <?php if ($relateditem->item_image) { ?>
        	<div class="djc_image">
        		<?php if ($this->params->get('image_link_item')) { ?>
					<a rel="lightbox-djitem-related" title="<?php echo $relateditem->image_caption; ?>" href="<?php echo DJCatalog2ImageHelper::getImageUrl($relateditem->item_image,'fullscreen'); ?>"><img alt="<?php echo $relateditem->image_caption; ?>" src="<?php echo DJCatalog2ImageHelper::getImageUrl($relateditem->item_image,'medium'); ?>"/></a>
				<?php } else { ?>
					<a href="<?php echo JRoute::_(DJCatalogHelperRoute::getItemRoute($relateditem->slug, (!isset($this->item) || (empty($this->item->catslug))) ? $relateditem->catslug:$this->item->catslug)); ?>"><img alt="<?php echo $relateditem->image_caption; ?>" src="<?php echo DJCatalog2ImageHelper::getImageUrl($relateditem->item_image,'medium'); ?>"/></a>
	        	<?php } ?>
        	</div>
		<?php } ?>
		<div class="djc_title">
	        <h3><?php
	          echo (JHTML::link(JRoute::_(DJCatalogHelperRoute::getItemRoute($relateditem->slug, (!isset($this->item) || (empty($this->item->catslug))) ? $relateditem->catslug:$this->item->catslug)), $relateditem->name));
	        ?></h3>
	    </div>
